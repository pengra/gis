...
class Command(BaseCommand):
    ...
    def _load_bg_vtd_map(self, state_fips):
        ...
        for polygon in polygons:
            ...
            newBg.save()
            newBg.subsection = StateSubsection.objects.filter(poly__touches=newBg.poly).order_by('-land_mass')[0]
            newBg.save()
    ...
    def _update_county_population(self, state_fips):
        ...
        for county in counties:
            county.population = sum([_.population for _ in StateSubsection.objects.filter(poly__touches=county.poly)])
            county.save()
            bar.next()
        bar.finish()
